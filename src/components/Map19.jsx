...
export const Map19 = ({
  ...
  const cityBuildings = useMemo(() => {
    ...
    // Depth layers: closer buildings (smaller Z) to farther buildings (larger Z)
    // Character is at z = -80, so buildings can be closer (negative Z) or behind (positive Z)
    // but avoid z = -80 to not interfere with character
    const depthLayers = [
      {
        z: -60, // Close foreground buildings (in front of character)
        count: 3,
        minHeight: 25,
        maxHeight: 40,
        minWidth: 10,
      ...
      {
        z: -20, // Near character level (but not on character axis)
        count: 5,
        minHeight: 35,
        maxHeight: 55,
        minWidth: 14,
      ...
      {
        z: 0, // At character level but behind
        count: 4,
        minHeight: 40,
        maxHeight: 60,
        minWidth: 14,
