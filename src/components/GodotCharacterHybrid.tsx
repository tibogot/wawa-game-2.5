...
export const GodotCharacterHybrid = ({
    ...
    "2.5dCamera": folder(
      {
        sideViewOffset: {
          ...
          step: 0.5,
          label:
            "Side View Distance (Z-axis) - Negative = Left Side, Positive = Right Side",
        },
        horizontalOffset: {
          value: 0,
          min: -10,
  ...
  // 2.5D mode: Store locked Z position
  const lockedZPosition = useRef<number | null>(null);

  // Reset locked Z position and initialize character rotation when entering/exiting 2.5D mode
  useEffect(() => {
    if (!is2_5DMode) {
      // Reset when exiting 2.5D mode
      lockedZPosition.current = null;
      // Reset rotation to forward-facing when exiting 2.5D mode
      rotationTarget.current = 0;
      characterRotationTarget.current = 0;
    } else {
      // When entering 2.5D mode, set character to face sideways (right side by default)
      // In 2.5D games like Limbo/Inside, character typically faces right initially
      rotationTarget.current = -Math.PI / 2; // -90 degrees = facing right
      characterRotationTarget.current = -Math.PI / 2;
    }
    // Note: We initialize lockedZPosition in useFrame on first frame when entering 2.5D mode
  }, [is2_5DMode]);
  ...
  useFrame((_state, delta) => {
    if (rb.current) {
      ...
      // Initialize locked Z position for 2.5D mode on first frame
      if (is2_5DMode && lockedZPosition.current === null && rb.current) {
        const pos = rb.current.translation();
        lockedZPosition.current = pos.z;
      }

      // Lock Z position and velocity in 2.5D mode
      if (is2_5DMode && lockedZPosition.current !== null && rb.current) {
        const pos = rb.current.translation();
        // Lock Z position
        if (Math.abs(pos.z - lockedZPosition.current) > 0.01) {
          rb.current.setTranslation(
            { x: pos.x, y: pos.y, z: lockedZPosition.current },
            true
          );
        }
        // Lock Z velocity
      ...
      // In 2.5D mode: disable forward/backward (Up/Down arrows), only allow left/right
      if (is2_5DMode) {
        // Only left/right movement in 2.5D mode
        if (get().left) movement.x = 1;
        if (get().right) movement.x = -1;
        // Forward/backward input is ignored in 2.5D mode
      } else {
        // Normal 3D movement
      ...
      if (movement.x !== 0) {
        if (is2_5DMode) {
          // In 2.5D mode, character rotation is based on left/right only
          // Character faces the direction of movement (left = facing left, right = facing right)
          rotationTarget.current = movement.x > 0 ? Math.PI / 2 : -Math.PI / 2;
        } else {
          // Normal 3D rotation
      ...
      if (movement.x !== 0 || movement.z !== 0) {
        ...
        if (is2_5DMode) {
          // 2.5D mode: Only X-axis movement (left/right)
          intendedVelX = movement.x * speed;
          intendedVelZ = 0; // Always zero in 2.5D mode
          characterRotationTarget.current =
            movement.x > 0 ? Math.PI / 2 : -Math.PI / 2;
        } else {
          // Normal 3D movement
          ...
          intendedVelX = Math.sin(movementRotation) * speed;
          intendedVelZ = Math.cos(movementRotation) * speed;
        ...
        if (grounded) {
          ...
          if (!is2_5DMode) {
            ...
            vel.z = 0; // Always lock Z in 2.5D mode
          }
      ...
      if (character.current) {
        ...
        // In 2.5D mode, character rotation is already set correctly (left/right facing)
        ...
        if (!is2_5DMode && cameraMode === "follow-orbit") {
    ...
    // CAMERA
    if (cameraMode === "2.5dcamera") {
      // 2.5D Side-view camera: Fixed side-view position looking at character from the side
      // Camera follows character on X and Y axes only, locked Z position
      if (!rb.current) return; // Early return if rigidbody not initialized

      const pos = rb.current.translation();

      // Lock Z position to character's initial Z (or locked Z if set)
      const characterZ =
        lockedZPosition.current !== null ? lockedZPosition.current : pos.z;

      // 2.5D side-view: Camera positioned to the side (along Z-axis offset) looking at character's side
      // Use dedicated 2.5D camera controls for positioning
      // For side view: camera is offset along Z-axis (depth) to see character from the side
      const targetCameraPos = new Vector3(
        pos.x + horizontalOffset, // Follow character X with horizontal offset (move camera left/right with character)
        pos.y + verticalOffset, // Follow character Y with vertical offset (move camera up/down with character)
        characterZ + sideViewOffset // Fixed Z offset from character (positive = camera is to the side, negative = other side)
      );
      ...
      // Look at character position (at locked Z, with vertical offset for look-at point)
      // Camera looks at the character from the side
      const lookAtPos = new Vector3(
        pos.x,
        pos.y + lookAtHeight, // Look at character with vertical offset
        characterZ
      );

      if (isFirstFrame) {
        cameraLookAt.current.copy(lookAtPos);
      } else {
        cameraLookAt.current.lerp(lookAtPos, cameraLerpSpeed);
      }

      // Camera always looks at character (side view)
      camera.lookAt(cameraLookAt.current);

      if (isFirstFrame) {
        cameraInitialized.current = true;
      }
    } else if (cameraMode === "follow" || cameraMode === "follow-orbit") {
